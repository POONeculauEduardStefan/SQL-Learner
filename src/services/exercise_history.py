...
from src.repositories.exercise_history import get_exercise_history_db, add_exercise_history_to_db, \
    ...
    get_exercise_history_by_user_db, get_exercises_stats_db
...
def get_exercise_history_by_user(db: Session, user_id: UUID):
    if find_user_by_id(str(user_id), db) is None:
