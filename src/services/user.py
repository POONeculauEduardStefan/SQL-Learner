...
from src.repositories.user import find_user_by_email, create_user, authenticate_user, update_user_password_db, \
    ...
    update_account_db, get_user_stats_db, save_user
...
def login_account(user: UserLoginSchema, db: Session):
    ...
    if existing_user is None or not existing_user.verified:
        raise AppException("User not found or not verified!", 404)
...
def reset_password(request: ResetPasswordRequest, db: Session):
    ...
    if info is None:
        raise AppException("Invalid Password Reset Payload or Reset Link Expired",
                           500)
...
def confirm_email(request: ConfirmEmailRequest, db: Session):
    info = decode_email_sent_token(request.secret_token)
    if info is None:
        raise AppException("Invalid Payload or Activation Link Expired",
                           500)
    user = find_user_by_email(info, db)
    if not user:
        raise AppException("User not found!", 404)
...
def get_user_by_email(email: str, db: Session):
    if not find_user_by_email(email, db):
        raise AppException("User doesn't exist!", 404)
...
def update_user_password(updated_user: UpdateUserPassword, email: str, db: Session):
    ...
    if not authenticate_user(updated_user.current_password.get_secret_value(), user.password, db):
        raise AppException("Current password is not correct!", 401)
...
async def update_user_image(image: UploadFile, email: str, db: Session):
    ...
    if image:
        ...
        raise AppException("No image found!", 404)
...
def get_all_users_paginated(db: Session, request: UsersPaginatedRequest, user_id: str):
    return get_all_users_paginated_in_db(db, request, user_id)
...
def delete_user_by_id(user_id: str, db: Session):
    ...
    if user.role == 1:
        raise AppException("Cannot delete admin user!", 403)
...
def promote_user_admin(user_id: str, db: Session):
    ...
    if not user:
        raise AppException("User doesn't exist!", 404)
    if user.role == 1:
        raise AppException("The user is already an admin!", 403)
