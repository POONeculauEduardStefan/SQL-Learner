...
from src.schemas.generator import GenerateAIExercise

ai_generator_router = APIRouter(prefix="/api/v1/generator", tags=["generator"])
OPENAI_API_KEY = config("OPENAI_API_KEY")
client = OpenAI(api_key=OPENAI_API_KEY)


@ai_generator_router.post("/", status_code=status.HTTP_201_CREATED)
def generate_quiz_question(topic: GenerateAIExercise):
    system_prompt = f"""
    Ești un profesor expert de SQL. Rolul tău este să creezi exerciții de practică. Fii foarte atent la correct query să nu aibe ; la final.
    ...
    Generează un singur exercițiu SQL care testează conceptul: {topic}.
    
    FORMATUL RĂSPUNSULUI (JSON strict):
    {{
        "question": "Enunțul exercițiului...",
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": "Generează un exercițiu nou."}
            ],
        ...
        print(f"AI ERROR: {e}")
        raise AppException("AI service unavailable", 500)
